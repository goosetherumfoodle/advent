...
cartAll' :: [[a]] -> [[a]] -> [[a]]
cartAll' (as:bs) acc = cartAll' bs ((cartTwo as) =<< acc)

cartTwo :: [a] -> [a] -> [[a]]
cartTwo as bs = cartTwo' as bs (as, bs) []

cartTwo' :: [a] -> [a] -> ([a], [a]) -> [[a]] -> [[a]]
cartTwo' (a:[]) (b:[]) _ acc = acc ++ [[a,b]]
cartTwo' (a:as) (b:[]) orig@(_,origBs) acc = cartTwo' as origBs orig (acc ++ [[a,b]])
cartTwo' (a:as) (b:bs) orig acc = cartTwo' (a:as) bs orig (acc ++ [[a,b]])
cartTwo' _ _ _ acc = acc
